...
**Why the evolved genomes are kept.** After 20-30 generations of Gaussian mutation the correlation
between `|w|` and the anatomical weights falls to 0.71-0.84, but the fraction of *exactly*
proportional entries is 0.0-0.2% -- chance level. The anatomical values are not recoverable from
them. Publishing them is what makes the ablation and tactics results checkable against the exact
...
proportionality against any graph's weights, the frozen suite may never be committed, the suite must
regenerate bit-identically, and every `np.load` must pass `allow_pickle=False`. The first two were
checked by reintroducing the offending file: the guard caught it at 100.0%.
